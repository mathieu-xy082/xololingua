...
function describeModelDelivery(capabilities) {
...
function createDemoStageRow({ stageName, stageCapabilities, offlineAvailability }) {
  ...
  if (stageCapabilities.modelDelivery) {
    row.modelDelivery = stageCapabilities.modelDelivery;
    row.modelRetention = stageCapabilities.modelRetention;
  }
  return row;
}
  ...
  if (offlineAvailability.backendRequiredStages.length === 0) {
    if (offlineAvailability.onlineRequiredStages.length > 0) {
      ...
      const onlineLabels = offlineAvailability.onlineRequiredStages
        .map((stageName) => PIPELINE_STAGE_LABELS[stageName])
        .join(", ");
      return `Offline shell assets available; ${offlineLabels} can run offline, but ${onlineLabels} needs online model delivery or a browser service.`;
    }
...
function requiresOnlineService(stageCapabilities = {}) {
  return stageCapabilities.strategy === "cloud-provider"
    || stageCapabilities.remoteModels === true
    || stageCapabilities.modelDelivery === "on-demand";
}
