...
This example targets one Ubuntu 24.04 server with 2 vCPU, 8 GiB RAM, no GPU, and one HTTPS domain. On the public site, the VPS performs CPU language identification only. Audio extraction, segmentation, transcription, and translation run in the visitor's browser; local development retains the Python fallbacks. The public service admits one language-identification request at a time, and excess work receives HTTP 429. Public videos are limited to 1 hour and the browser extraction path accepts MP4 files up to 400 MiB. A separate capacity and abuse review is needed before promoting this to a high-traffic service.
...
7. Add the site block from `deploy/Caddyfile.example` to `/etc/caddy/Caddyfile`, replacing the example domain. Run `sudo caddy validate --config /etc/caddy/Caddyfile`, then `sudo systemctl reload caddy`. Verify `https://YOUR_DOMAIN/`, `https://YOUR_DOMAIN/api/health`, and an actual small MP4 end-to-end from a different machine. Confirm that `GET /api/subtitle-jobs` returns 404, `POST /api/subtitle-jobs` returns 403, and that the browser's Network tab shows only language identification using the processing API.

## Limits and operations

- `XOLOLINGUA_PUBLIC_MODE=1` disables the global job listing, cross-origin API access, and all Python processing endpoints except `POST /api/detect-language`. It limits identification to one active request and 12 requests per IP per hour. Caddy sets `X-Real-IP` itself so visitors cannot choose their own rate-limit identity. Visitors sharing one public IP also share that allowance.
- Public mode rejects videos longer than 3,600 seconds after probing the uploaded media and removes the rejected temporary file. The site UI checks the same duration limit and also rejects MP4 files larger than 400 MiB before upload. Local development keeps the 9,000-second limit for long-video tests and its Python processing fallbacks.
- The Python API caps video requests at 400 MiB plus 1 MB for multipart framing, even if `XOLOLINGUA_PUBLIC_MAX_UPLOAD_BYTES` is higher. Set the live Caddy `request_body /api/*` limit to `421MB` as in `deploy/Caddyfile.example` and reload Caddy after validating its config. The Python service also caps its media directory at 10 GB with 1 GB of headroom for identification. It removes abandoned uploaded media older than 24 hours on the next upload. Allow room for temporary upload files, installed models, and OS updates within the 68 GiB free disk.
- `journalctl -u xololingua -f` shows Python errors; `journalctl -u caddy -f` shows HTTPS and reverse-proxy errors. A 429 means the CPU identification slot or hourly request allowance was reached. A 403 on a processing endpoint means the public browser-only policy blocked it. A 413 means the MP4 exceeded the API upload limit.
